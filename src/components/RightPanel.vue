<template>
	<div :class="['right-board', visible ? 'show' : '']">
		<el-scrollbar class="right-scrollbar">
			<!-- 组件属性 -->
			<div v-if="activeData.__config__.name !== undefined" class="title">
			...
			>
				<el-form-item
					v-if="
						!(!isPC && activeData.__mobile__.noLabel) &&
				...
				<el-form-item
					v-if="
						!isPC &&
							activeData.__mobile__.props.startLabel !== undefined
					"
				...
				<el-form-item
					v-if="
						!isPC &&
							activeData.__mobile__.props.endLabel !== undefined
					"
				...
				<el-form-item
					v-if="
						!isPC &&
							activeData.__mobile__.props.placeholder !==
								undefined
					"
					label="提示文字"
				>
				...
				<el-form-item
					v-if="
						isPC &&
							activeData.__pc__.props['start-placeholder'] !==
								undefined
					"
					label="开始提示文字"
				>
				...
				<el-form-item
					v-if="
						[
							...
							'el-radio-group',
							'el-select',
						].indexOf(activeData.__pc__.tag) > -1
					"
				...
				<el-form-item label="其他">
					<el-checkbox
						v-if="
							activeData.__mobile__.props.autoCalc !== undefined
						"
					...
					<el-checkbox
						...
						:class="{
							'required-checked__condition':
								activeData.__config__.proCondition,
						}"
				...
				<el-form-item
					...
					label="字段占比"
				>
					<el-radio v-model="activeData.__pc__.span" :label="12">
						50%
					</el-radio>
					<el-radio v-model="activeData.__pc__.span" :label="24">
						100%
					</el-radio>
				</el-form-item>
				<el-form-item
				...
				>
					...
					<el-radio v-model="activeData.__pc__.tag" label="el-select">
						下拉模式
					</el-radio>
				</el-form-item>
				<!-- <el-form-item
				...
				return [
					{
						validator: (rule, value = [], callback) => {
							if (!value.length) {
								...
								value.some((item) => item.label === '')
							) {
								callback(new Error('不能有空的选项'));
							} else if (
								new Set(value.map(JSON.stringify)).size <
								value.length
							) {
